// ════════════════════════════════════════════════════════════
//  ROUNDER Admin — Notification Management
...
class _AdminNotificationsScreenState extends State<AdminNotificationsScreen>
  ...
  Widget _buildHeader() {
    return const Row(
      children: [
        Text('알림 관리', style: AdminTextStyles.pageTitle),
        SizedBox(width: 12),
        AdminBadge(label: '정책 · 알림톡 · 푸시', color: AdminColors.statusInfo),
      ],
    );
  }
}
...
// ────────────────────────────────────────────────────────────
//  Notification Policy Tab (첨부 표)
// ────────────────────────────────────────────────────────────
class _NotificationPolicyTab extends StatelessWidget {
  const _NotificationPolicyTab();

  @override
  Widget build(BuildContext context) {
    final rows = AdminCatalog.notificationPolicies;
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            '가입·일정·회비 알림의 채널 / 수신 대상 / 타이밍 기준입니다.',
          ...
          Expanded(
            child: SingleChildScrollView(
              child: Table(
                ...
                border: TableBorder.all(color: AdminColors.cardBorder),
                children: [
                  TableRow(
                    decoration: const BoxDecoration(color: Color(0xFFF8FAFC)),
                    children: [
                      _cell('번호', header: true),
                      _cell('알림 상황', header: true),
                      _cell('채널', header: true),
                      _cell('수신 대상', header: true),
                      _cell('타이밍', header: true),
                    ],
                  ),
                  ...rows.map(
                    (r) => TableRow(
    ...
    return Padding(
      ...
      child: Text(
        ...
        style: TextStyle(
          ...
          color: header ? AdminColors.textSecond : AdminColors.textPrimary,
        ),
      ),
    );
  }
